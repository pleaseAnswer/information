...
1. `typeof`：判断基本数据类型
2. `Object.prototy.toString.call()`：判断数据类型
...
* 闭包就是引用了其他函数作用域中变量的函数，这种模式通常在函数嵌套结构中实现。
...
* **事件委托**
  * 在父级元素上监听事件，通过事件冒泡，父元素可以监听到子元素上事件的触发，通过判断事件发生元素DOM的类型，来做出不同的响应
